jQuery(document).ready(function($) {
	...
	$(document).ajaxSuccess(function(e, xhr, options){
		if( options.data.search( 'action=save-widget' ) != -1 ) {
			...
			// CI Handpicked
			if( ( widget_id = options.data.match( /widget-id=(ci-handpicked-\d+)/ ) ) !== null ) {
			...
			// CI Split Custom
			if( ( widget_id = options.data.match( /widget-id=(ci-split-custom-\d+)/ ) ) !== null ) {
				var widget = $("input[name='widget-id'][value='" + widget_id[1] + "']").parent();
				ci_collapsible_init( widget );
			}
			...
			// CI Testimonials
			if( ( widget_id = options.data.match( /widget-id=(ci-testimonial-\d+)/ ) ) !== null ) {
				var widget = $("input[name='widget-id'][value='" + widget_id[1] + "']").parent();
				ci_collapsible_init( widget );
			}
			...
			// CI Events
			if( ( widget_id = options.data.match( /widget-id=(ci-book-event-\d+)/ ) ) !== null ) {
				var widget = $("input[name='widget-id'][value='" + widget_id[1] + "']").parent();
				ci_collapsible_init( widget );
			}

			// CI Events
			if( ( widget_id = options.data.match( /widget-id=(ci-info-\d+)/ ) ) !== null ) {
				var widget = $("input[name='widget-id'][value='" + widget_id[1] + "']").parent();
				ci_collapsible_init( widget );
			}
	...
	$('body').on('change', '.ci-repeating-fields .posttype_dropdown', function(){
		...
		$.ajax({
			...
			data: {
				action: 'ci_handpicked_widget_post_type_ajax_get_posts',
	...
	$('body').on('change', '.uploaded-id', function(){
		var preview = $(this).siblings('.selected_image');
		$.ajax({
			type: "post",
			url: ThemeWidget.ajaxurl,
			data: {
				action: 'ci_split_custom_widget_ajax_get_selected_image',
				image_id: $(this).val(),
